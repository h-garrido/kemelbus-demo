...
export default function SeatSelectionView({
  ...
  const [selectedFareType, setSelectedFareType] = useState<string>(
    fares.length > 0 ? fares[0].fare_type : ""
  );
  ...
  const getSeatStatusText = (seat: Seat) => {
    if (cartSeatIds.has(seat.id))     return 'en el carrito';
    if (seat.status !== 'available')  return 'ocupado';
    if (selectedSeat?.id === seat.id) return 'seleccionado';
    return 'disponible';
  };

  const SeatButton = ({ seat }: { seat: Seat }) => {
    const statusText = getSeatStatusText(seat);
    const activePrice = fares.find(f => f.fare_type === selectedFareType)?.price ?? seat.price;
    const ariaLabel = `Asiento número ${seat.seat_number}, tipo ${seat.type}, ${statusText}. Precio: $${activePrice.toLocaleString('es-CL')} pesos.`;
    ...
    return (
      <button
        ...
        className={`relative h-11 rounded-md flex flex-col items-center justify-center gap-0.5 transition-all duration-200 cursor-pointer ${getSeatClass(seat)}`}
      >
        <Armchair size={14} />
      ...
      <section className="py-16 max-w-7xl mx-auto px-6">
        <div className="bus-frame">
          <div className="p-8 md:p-12 grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            {/* Mapa de asientos */}
            <div className="bus-seat-map" aria-label="Mapa interactivo de distribución de asientos en el bus">
            ...
            <div className="flex flex-col justify-center space-y-6">
                ...
                {selectedSeat ? (
                  <div>
                    ...
                    <button
                      ...
                      className="text-xs text-gray-400 hover:text-red-400 font-semibold transition-colors cursor-pointer"
                    >
                      Cambiar selección ✕
                    </button>
                  </div>
              ...
              <button
                ...
                data-confirm-button
                className="btn-primary w-full py-4 flex items-center justify-center gap-2 group cursor-pointer
